...
exports.workbook = {
    ...
    text_function: function(test){
        var v,i,m;
        test.expect(text_queries.length);
        var workbook = new Workbook();
        var sheet = workbook.createWorkSheet();
        for(i=0,m=cells.length;i<m;i+=1){
            sheet.getCell(cells[i].id,cells[i]);
        }
        for(i=0,m=text_queries.length;i<m;i+=1){
            sheet.getCell('B'+(i+1)).formula=text_queries[i].test;
            test.equals(sheet.getCell('B'+(i+1)).value, text_queries[i].result,text_queries[i].test);
        }
        test.done();
    }
}