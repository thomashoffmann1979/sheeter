		...
		if (intend>0){
			...
			if (
				...
				try{
					...
					result_tokens.add(cell.worksheet.getCell(token.value).value,"literal",keywords.subtype.number);
				}catch(error){
					//console.log(error);
	...
	try{
		if(token_list.moveNext()){
			...
			if (cell.worksheet.workbook.functionStore.has(current.value)){
				...
				return cell.worksheet.workbook.functionStore.get(current.value)(token_list);
			}else{
				return '#N/A';
			}
		}else{
