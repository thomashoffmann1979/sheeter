...
function execute(token_list){
	...
	while (token_list.moveNext()) {
		...
		if (intend>0){
			...
			if (
				...
				try{
					result_tokens.add(cell.worksheet.getCell(token.value).value,"literal","number");
				}catch(error){
					//console.log(error);
					result_tokens.add('#NV',"literal","error");
				}
			}else if (
				(token.type==='literal') &&
				(token.subtype==='number')  // change here to support, strings and so on ...
			){
				result_tokens.add(token.value,"literal","number");
			}else{
		...
		if ( (token.type==='subexpression') && (token.subtype==='stop')){
			...
			if (intend===0){
				...
				subtokens= new TokenList()
				result_tokens.add(calculated_value,"literal","number");
			}
		}
	...
	while(result_tokens.moveNext()){
		console.log(result_tokens.current())
	}
	//console.log(result_tokens._items);
