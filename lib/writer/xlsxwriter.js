...
var XLSXWriter = function (config){
    ...
    this._zip = new JSZip();  //new MZip();//new Zip();
    
    this.folder_rels = this._zip.folder('_rels');
    this.folder_xl = this._zip.folder('xl' );
    this.folder_docProps = this._zip.folder('docProps' );
    this.folder_theme = this.folder_xl.folder('theme' );
    this.folder_worksheets = this.folder_xl.folder('worksheets' ); 
    this.folder_xl_rels = this.folder_xl.folder('_rels' ); 
...
XLSXWriter.prototype.documentRelationReady = function(wr){
    //console.log(wr.data);
    console.log('done');
    var b64 = this._zip.generate({ compression: 'DEFLATE' });
    fs.writeFileSync(this.filename, new Buffer(b64,'base64'));
    //this.zip.writeZip(this.filename);
}
//me.emit('sheetsReady');
